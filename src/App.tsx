...
function Catalog({ courses, enrolledIds, hasPlus, enroll }: { courses: Course[]; enrolledIds: string[]; hasPlus: boolean; enroll: (course: Course) => void }) {
  ...
  return <main className="page"><header className="page-hero mint"><span className="eyebrow dark">Course catalog</span><h1>Choose what you want to build next.</h1><p>Enroll immediately. No application is required for open courses.</p></header><section className="section catalog"><div className="filters">{schools.map((school) => <button key={school} className={filter === school ? "active" : ""} onClick={() => setFilter(school)}>�~-�G����ƭy�</div>}</section></main>;
}
